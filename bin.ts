...
const resolveDependencies = async (dirpath: string, dependencies: Array<string>): Promise<Array<string>> => {
    return Promise.all(
        dependencies.map(
...
const hashDependencies = async (dirpath: string, dependencyPaths: Array<string>): Promise<DependencyHashes> => {
    return Promise.all(
        dependencyPaths.map(
...
const hashesToRecord = (dependencyHashes: DependencyHashes): Record<string, string> => {
...
const checkManifest = async (dirpath: string, targetOutput: string, dependencyHashes: DependencyHashes): Promise<boolean> => {
    let rawManifest;
    try {
...
const writeManifest = async (dirpath: string, targetOutput: string, dependencyHashes: DependencyHashes): Promise<void> => {
...
const run = async (dirpath: string, targetName: string | undefined): Promise<void> => {
